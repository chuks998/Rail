...
def dashboard(request):
    ...
    # deposit = Deposit.objects.filter(account_holder=user)
    withdraw = Withdraw.objects.filter(sender=user)
    return render(request, 'dashboard.html', {'details':accounts, 'history':history, 'deposit':deposit, 'withdraw':withdraw})
    ...
    if request.method == 'POST':
        ...
        if auth_user:
            try:
                ...
                model_1 = AccountDetail.objects.get(account_number=reciver_account_number)

                if model_1:
                    model_2 = Transfer.objects.all()
                    if auth_reciver.username == reciver_username:
                        model_2.create(sending_user=sender, reciver_account=reciver_account_number, reciver_username=reciver_username, amount=transfer_amount)
                        if model_2:
                            sender_detail = AccountDetail.objects.get(account_holder=sender)
                            reciver_detail = AccountDetail.objects.get(account_number=reciver_account_number)
                            if sender_detail.account_balance >= float(transfer_amount):

                                sender_detail.account_balance = F('account_balance') - float(transfer_amount)
                                reciver_detail.account_balance = F('account_balance') + float(transfer_amount)
        ...
        if account_owner.account_verified:
            if account_owner.account_ready:
                ...
                if account_owner.account_balance < float(amount):
                    ...
                    model.create(sender=request.user, amount=amount, account_number=account_number, bank_name=bank_name)
